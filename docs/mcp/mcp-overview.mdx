---
title: "MCP Overview"
description: "Learn about Model Context Protocol (MCP) servers, their capabilities, and how LUMI can help build and use them. MCP standardizes how applications provide context to LLMs, acting like a USB-C port for AI applications."
---
...
<Frame>
	<img
		src="https://storage.googleapis.com/dietcode_public_images/docs/assets/mcp-diagram.png"
...
-   **Security** depends on trusted server configuration and LUMI's execution policy. MCP tool calls use the same funnel as other tools; autonomous mode removes per-tool consent prompts while execution policy, hooks, workspace/lane authority, and MCP settings continue to govern dispatch.
...
LUMI does not come with any pre-installed MCP servers. You'll need to find and install them separately.
...
-   **LUMI Marketplace:** Install one from LUMI's [MCP Marketplace](/mcp/mcp-marketplace)
-   **Ask LUMI:** You can ask LUMI to help you find or create MCP servers
...
## Integration with LUMI

MCP servers work with the **LUMI VS Code extension**.

LUMI simplifies the building and use of MCP servers through its AI capabilities.

### Building MCP Servers

-   **Natural language understanding:** Instruct LUMI in natural language to build an MCP server by describing its functionalities. LUMI will interpret your instructions and generate the necessary code.
-   **Cloning and building servers:** LUMI can clone existing MCP server repositories from GitHub and build them automatically.
-   **Configuration and dependency management:** LUMI handles configuration files, environment variables, and dependencies.
-   **Troubleshooting and debugging:** LUMI helps identify and resolve errors during development.

### Using MCP Servers

-   **Tool execution:** LUMI seamlessly integrates with MCP servers, allowing you to execute their defined tools.
-   **Context-aware interactions:** LUMI can intelligently suggest using relevant tools based on conversation context.
-   **Dynamic integrations:** Combine multiple MCP server capabilities for complex tasks. For example, LUMI could use a GitHub server to get data and a Notion server to create a formatted report.
...
## Implementation in LUMI

| Component | Path | Role |
|-----------|------|------|
| Connection hub | `src/services/mcp/McpHub.ts` | Spawns servers, lists tools/resources, routes calls |
| Tool handler | `src/core/task/tools/handlers/UseMcpToolHandler.ts` | Declares pure MCP intent and executes only under the funnel permit |
| Resource handler | `src/core/task/tools/handlers/AccessMcpResourceHandler.ts` | Reads MCP resources into context |
| Remote add | `src/core/controller/mcp/addRemoteMcpServer.ts` | Adds SSE/HTTP servers from UI |

MCP server configs live in user/workspace settings and sync via `src/core/storage/remote-config/syncRemoteMcpServers.ts`. LUMI ships with **no pre-installed servers** — you add them via [Adding and Configuring Servers](/mcp/adding-and-configuring-servers) or the MCP Marketplace.

See [SECURITY_BEST_PRACTICES](/SECURITY_BEST_PRACTICES) for consent boundaries on MCP tool calls.
