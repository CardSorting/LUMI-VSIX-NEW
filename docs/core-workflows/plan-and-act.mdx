...
4. The agent transitions to Act when the plan is ready; the user can steer or refine it at any point.
5. In autonomous Act mode, the agent executes in-scope operations without per-tool approval. Policy, hooks, workspace/lane authority, cancellation, and lifecycle checks continue to govern dispatch.
...
In autonomous mode, `ExecutionFunnel` proceeds with in-scope operations without per-tool consent prompts. Execution policy, hooks, workspace/lane authority, cancellation, and lifecycle checks still apply before a permit is issued. With autonomous mode off, Auto Approve settings and handler eligibility govern automatic execution. See [auto-approve](/features/auto-approve).
