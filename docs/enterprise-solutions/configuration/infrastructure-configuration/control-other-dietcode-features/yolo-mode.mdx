...
YOLO Mode lets LUMI execute agent-selected operations without per-tool consent prompts. Execution hooks, operation policy, workspace and lane authority, and lifecycle checks remain active. For Enterprise administrators, this page covers how to control access to YOLO Mode across your organization.
...
When YOLO Mode is enabled, file changes, terminal commands, browser actions, and delegated work proceed without a per-tool consent prompt. Consent declarations remain in the audit record; execution hooks, workspace fencing, command policy, and completion checks remain active.
...
- Users can enable or disable YOLO Mode in their local LUMI settings
...
<AccordionGroup>
  <Accordion title="Security & Compliance" icon="shield">
    YOLO Mode can run eligible operations without a per-tool prompt, which can allow:
  ...
  <Accordion title="Code Quality Control" icon="code">
    ...
    - Human review may happen after eligible changes have run; task completion gates remain active
...
**Risk Factors:**
- Eligible operations can run without a per-tool prompt; handler-declared consent requirements still pause for review
- Potential for rapid propagation of mistakes across multiple files
- Review may happen after an eligible change is applied rather than before it
- Autonomous execution does not replace organization-specific command policies, hooks, or access controls
...
- Join our [Discord](https://discord.gg/dietcode) for community discussion
