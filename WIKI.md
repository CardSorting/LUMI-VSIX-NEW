...
| Guided Spec Mode (`AUTO`) | `src/shared/guidedSpec/`, `webview-ui/src/components/chat/GuidedSpecCard.tsx` | Deterministic execution wrapper layered over core LLM engine under `AUTO` mode. Suppresses raw code and terminal syntax in favor of visual Breadboard maps, Milestone stepper progress timelines, 1-click decision chips (`Option A`/`Option B`), Superhuman single-key shortcuts (`Press A`/`Press B`), and an interactive Product Architecture Canvas. |
