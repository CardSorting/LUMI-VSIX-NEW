...
| Model outputs raw code blocks or terminal commands when in AUTO mode | System prompt override not injected or provider model ignoring zero-syntax directives | Check if mode is set to "auto" and inspect PromptBuilder system prompt | Ensure `mode === "auto"` is active; re-select **AUTO** mode on `ModModeSwitcher` bar | `parseGuidedSpecOutput` parses 4-block layout; GuidedSpecCard renders visual breadboard cards |
