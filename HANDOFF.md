...
| Guided Spec Mode (`AUTO`) | Deterministic execution wrapper layered over core LLM engine. Suppresses raw code/terminal syntax for non-technical stakeholders in favor of visual Breadboard surface maps, Progress Stepper timelines (`2/3 - 66%`), 1-Click Decision Chips (`Option A`/`Option B`), Superhuman single-key shortcuts (`Press A`/`Press B`), Decision Rationale trade-off drawers (`💡 Why Option A is Recommended`), an interactive **Product Architecture Canvas Drawer**, and sticky 1-click composer quick approval bars ([ADR-003](docs/architecture/adr-003-guided-spec-execution-mode.md), [ADR-017](DECISIONS.md#adr-017-guided-spec-execution-mode-v100-spec--zero-syntax-product-management)) | `src/shared/guidedSpec/`, `webview-ui/src/components/chat/GuidedSpecCard.tsx`, `webview-ui/src/components/chat/ChatTextArea.tsx`, `webview-ui/src/components/chat/ModModeSwitcher.tsx` |
