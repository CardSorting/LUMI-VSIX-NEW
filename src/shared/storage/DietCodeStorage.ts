...
export interface DietCodeStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: DietCodeStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and DietCodeSyncStorage both satisfy this interface.
 */
export interface DietCodeMemento {
 ...
 */
export abstract class DietCodeStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "DietCodeStorage"
	/**
 ...
 * Unlike DietCodeStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: DietCodeStorageChangeEvent) => void

export abstract class DietCodeSyncStorage<T = any> {
...
/**
 * A simple in-memory implementation of DietCodeStorage using a Map.
 */
export class DietCodeInMemoryStorage extends DietCodeStorage {
	/**
