...
import { DietCodeAssistantThinkingBlock, DietCodeStorageMessage } from "@/shared/messages/content"
...
export function addReasoningContent(
	...
	originalMessages: DietCodeStorageMessage[],
		...
		if (msg.role === "assistant") {
			if (Array.isArray(msg.content)) {
				const thinking = msg.content
					.filter((p): p is DietCodeAssistantThinkingBlock => p.type === "thinking")
					.map((p) => p.thinking)
					.join("\n")
				if (thinking) {
