...
import { DIETCODE_RECOMMENDED_MODELS_FALLBACK } from "@/shared/dietcode/recommended-models"
import { FeatureFlag } from "@/shared/services/feature-flags/feature-flags"
import { Logger } from "@/shared/services/Logger"
import {
	refreshDietCodeRecommendedModels,
	resetDietCodeRecommendedModelsCacheForTests,
} from "../refreshDietCodeRecommendedModels"

describe("refreshDietCodeRecommendedModels", () => {
	...
	it("returns hardcoded models and skips upstream fetch when rollout flag is off", async () => {
		...
		const result = await refreshDietCodeRecommendedModels()

		expect(result).to.deep.equal(DIETCODE_RECOMMENDED_MODELS_FALLBACK)
	...
	it("fetches from upstream when rollout flag is on", async () => {
		sandbox.stub(getFeatureFlagsService(), "getBooleanFlagEnabled").callsFake((flag) => {
			return flag === FeatureFlag.DIETCODE_RECOMMENDED_MODELS_UPSTREAM
		})
	...
	it("uses hardcoded models when rollout flag is turned off after upstream cache is populated", async () => {
		...
		const firstResult = await refreshDietCodeRecommendedModels()
		const secondResult = await refreshDietCodeRecommendedModels()

		expect(axiosGetStub.calledOnce).to.equal(true)
		expect(firstResult).to.not.deep.equal(DIETCODE_RECOMMENDED_MODELS_FALLBACK)
		expect(secondResult).to.deep.equal(DIETCODE_RECOMMENDED_MODELS_FALLBACK)
	})
})
