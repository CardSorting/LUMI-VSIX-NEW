...
describe("updateApiConfigurationPartial", () => {
	it("preserves the Codex provider when a later credential update omits provider fields", async () => {
		...
		await updateApiConfigurationPartial(
			...
			UpdateApiConfigurationPartialRequest.create({
				apiConfiguration: { actModeApiProvider: ProtoApiProvider.OPENAI_CODEX },
		...
		await updateApiConfigurationPartial(
			controller,
			UpdateApiConfigurationPartialRequest.create({
				apiConfiguration: { planModeApiModelId: "gpt-5.4" },
				updateMask: ["planModeApiModelId"],
			}),
		)

		expect(configuration.actModeApiProvider).to.equal("openai-codex")
		expect(configuration.planModeApiModelId).to.equal("gpt-5.4")
	})
