...
/**
 * The raw model information returned by the DietCode API to list models
 */
interface DietCodeRawModelInfo {
	id: string
	name: string
...
async function fetchRawDietCodeModels(): Promise<DietCodeRawModelInfo[]> {
	const apiBaseUrl = DietCodeEnv.config().apiBaseUrl
	const response = await axios.get(`${apiBaseUrl}/api/v1/ai/dietcode/models`, getAxiosSettings())

	if (!Array.isArray(response.data?.data)) {
		throw new Error("Invalid response data when fetching DietCode models")
	}

	Logger.log("DietCode models source: DietCode API")
	return response.data.data as DietCodeRawModelInfo[]
}

/**
 * Core function: Refreshes the DietCode models and returns application types
 * @param controller The controller instance
 * @returns Record of model ID to ModelInfo (application types)
 */
export async function refreshDietCodeModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	const shouldUseDietCodeEndpointSource = featureFlagsService.getBooleanFlagEnabled(
	...
	const cache = StateManager.get().getModelsCache("dietcode")
	if (cache) {
		return cache
	}
	...
	pendingRefresh = (async () => {
		try {
			return await fetchAndCacheDietCodeModels()
		} finally {
...
async function fetchAndCacheDietCodeModels(): Promise<Record<string, ModelInfo>> {
	const dietcodeModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.dietcodeModels)
	...
	try {
		...
		if (Object.keys(models).length === 0) {
			throw new Error("No DietCode models returned from API")
		}
		// Save models and cache them in memory
		await fs.writeFile(dietcodeModelsFilePath, JSON.stringify(models))
		Logger.log("DietCode models fetched and saved")
	} catch (error) {
		Logger.error("Error fetching DietCode models:", error)

		// If we failed to fetch models, try to read cached models from disk
		try {
			const fileExists = await fileExistsAtPath(dietcodeModelsFilePath)
			if (fileExists) {
				const fileContents = await fs.readFile(dietcodeModelsFilePath, "utf8")
				models = JSON.parse(fileContents)
				Logger.log("Loaded DietCode models from cache")
			}
		} catch (cacheError) {
			Logger.error("Error reading DietCode models from cache:", cacheError)
		}
	}

	// Avoid poisoning in-memory cache with an empty model map after transient failures.
	if (Object.keys(models).length > 0) {
		StateManager.get().setModelsCache("dietcode", models)
	}
...
/**
 * Read cached DietCode models from disk
 * @returns The cached models or undefined if not found
 */
export async function readDietCodeModelsFromCache(): Promise<Record<string, ModelInfo> | undefined> {
	try {
		const dietcodeModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.dietcodeModels)
		const fileExists = await fileExistsAtPath(dietcodeModelsFilePath)
		if (fileExists) {
		...
		Logger.error("Error reading DietCode models from cache:", error)
	}
	return undefined
}
