...
import type { DietCodeSayGenerateExplanation } from "@/shared/ExtensionMessage"
...
/**
 * Helper to create a stringified DietCodeSayGenerateExplanation message
 */
function createExplanationMessage(
	title: string,
	fromRef: string,
	toRef: string,
	status: DietCodeSayGenerateExplanation["status"],
	error?: string,
): string {
	const message: DietCodeSayGenerateExplanation = { title, fromRef, toRef, status }
	if (error) {
		message.error = error
	}
