...
import { DietCodeApiReqCancelReason, DietCodeApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const dietcodeMessages = params.messageStateHandler.getDietCodeMessages()
	const currentApiReqInfo: DietCodeApiReqInfo = JSON.parse(dietcodeMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateDietCodeMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
