...
type MessageStateHandlerLike = {
	getDietCodeMessages(): DietCodeMessageLike[]
}
...
/**
 * Builds the evaluation context used for conditional DietCode Rules (e.g. YAML frontmatter `paths:`).
 *
 * Kept in the user-instructions domain so Task remains orchestration-focused.
 *
 * Path context is gathered from multiple sources in dietcodeMessages:
...
export class RuleContextBuilder {
	...
	private static async getRulePathContext(deps: RuleContextBuilderDeps): Promise<string[]> {
		...
		const dietcodeMessages = deps.messageStateHandler.getDietCodeMessages()
		...
		const lastUserMsg = [...dietcodeMessages]
			.reverse()
		...
		const rawVisiblePaths = visibleTabs?.paths ?? []
		const rawOpenTabPaths = openTabs?.paths ?? []
		...
		// (3) Files edited by DietCode during this task (completed operations):
