...
describe("DietCodeIgnoreController", () => {
	let tempDir: string
	let controller: DietCodeIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .dietcodeignore file
		await fs.writeFile(
			path.join(tempDir, ".dietcodeignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new DietCodeIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .dietcodeignore file", async () => {
			const result = controller.validateAccess(".dietcodeignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .dietcodeignore", async () => {
			// Create a new .dietcodeignore with comments
			await fs.writeFile(
				path.join(tempDir, ".dietcodeignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .dietcodeignore gracefully", async () => {
			// Create a new controller in a directory without .dietcodeignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new DietCodeIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .dietcodeignore", async () => {
			await fs.writeFile(path.join(tempDir, ".dietcodeignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .dietcodeignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".dietcodeignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .dietcodeignore
			controller = new DietCodeIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .dietcodeignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .dietcodeignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".dietcodeignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".dietcodeignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
