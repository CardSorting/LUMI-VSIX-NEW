...
export class PromptBuilder {
	...
	async build(): Promise<string> {
		...
		const reliableWorkingStyle = [
			"## Working style",
			"- Make the smallest change that fully addresses the request.",
			"- Inspect relevant code before editing; preserve unrelated behavior and structure.",
			"- Prefer direct, well-established approaches over new abstractions or speculative refactors.",
			"- Be explicit about uncertainty and report what was and was not verified.",
		].join("\n")
		return this.postProcess(`${executionStateHeader}${prompt}\n\n${reliableWorkingStyle}`)
	}
