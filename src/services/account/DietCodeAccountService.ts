...
export class DietCodeAccountService {
	private static instance: DietCodeAccountService
	...
	/**
	 * Returns the singleton instance of DietCodeAccountService
	 * @returns Singleton instance of DietCodeAccountService
	 */
	public static getInstance(): DietCodeAccountService {
		if (!DietCodeAccountService.instance) {
			DietCodeAccountService.instance = new DietCodeAccountService()
		}
		return DietCodeAccountService.instance
	}

	/**
	 * Returns the base URL for the DietCode API
	 * @returns The base URL as a string
	 */
	get baseUrl(): string {
		return DietCodeEnv.config().apiBaseUrl
	}
	...
	private async authenticatedRequest<T>(endpoint: string, config: AxiosRequestConfig = {}): Promise<T> {
		...
		const dietcodeAccountAuthToken = await this._authService.getAuthToken()
		if (!dietcodeAccountAuthToken) {
			throw new Error("No DietCode account auth token found")
		}
		const requestConfig: AxiosRequestConfig = {
			...config,
			headers: {
				Authorization: `Bearer ${dietcodeAccountAuthToken}`,
	...
	async fetchMe(): Promise<UserResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<UserResponse>(DIETCODE_API_ENDPOINT.USER_INFO)
			return data
		} catch (error) {
	...
	async switchAccount(organizationId?: string): Promise<void> {
		...
		try {
			...
			const _response = await this.authenticatedRequest<string>(DIETCODE_API_ENDPOINT.ACTIVE_ACCOUNT, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
