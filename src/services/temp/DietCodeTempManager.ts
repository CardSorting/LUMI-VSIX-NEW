/**
 * DietCodeTempManager - Manages temporary files for DietCode with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "dietcode" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for DietCode's temporary files.
 */
class DietCodeTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "dietcode" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/dietcode
		// Windows: C:\Users\{user}\AppData\Local\Temp\dietcode
		// Linux: /tmp/dietcode
		const baseTempDir = os.tmpdir()
		const dietcodeTempDir = path.join(baseTempDir, "dietcode")

		try {
			fs.mkdirSync(dietcodeTempDir, { recursive: true })
			this.tempDir = dietcodeTempDir
		} catch {
	...
	/**
	 * Clean up old DietCode temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the DietCode temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(
					`DietCode temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`,
				)
			}
		} catch (error) {
			Logger.error("Error during DietCode temp cleanup", error)
		}
...
export const DietCodeTempManager = new DietCodeTempManagerImpl()
