...
export interface DietCodeAuthApiTokenExchangeResponse {
	success: boolean
	data: DietCodeAuthResponseData
}

export interface DietCodeAuthApiTokenRefreshResponse {
	success: boolean
	data: DietCodeAuthResponseData
}
...
export class DietCodeAuthProvider implements IAuthProvider {
	...
	private clearSession(controller: Controller, reason: string, storedAuthData?: DietCodeAuthInfo) {
		Logger.error(reason)
		...
		controller.stateManager.setSecret("dietcode:dietcodeAccountId", undefined)
		this.refreshRetryCount = 0
		this.lastRefreshAttempt = 0
		return null
	}

	private logFailedRefreshAttempt(response: Response, storedAuthData?: DietCodeAuthInfo) {
		const startedAt = storedAuthData?.startedAt
		const timeSinceStarted = Date.now() - (startedAt || 0)
	...
	/**
	 * Retrieves DietCode auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<DietCodeAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveDietCodeAuthInfo(controller: Controller): Promise<DietCodeAuthInfo | null> {
		try {
			...
			const storedAuthDataString = controller.stateManager.getSecretKey("dietcode:dietcodeAccountId")
			...
			let storedAuthData: DietCodeAuthInfo
			try {
			...
			if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
				...
				try {
					...
					if (newAuthInfoString !== storedAuthDataString) {
						controller.stateManager.setSecret("dietcodeAccountId", undefined) // cleanup old key
						controller.stateManager.setSecret("dietcode:dietcodeAccountId", newAuthInfoString)
					}
					...
					// when the user actually tries to use DietCode, not at startup
					return storedAuthData
				}
			}
	 ...
	 * @returns {Promise<DietCodeAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string, storedData: DietCodeAuthInfo): Promise<DietCodeAuthInfo> {
		try {
			const endpoint = new URL(DIETCODE_API_ENDPOINT.REFRESH_TOKEN, this.config.apiBaseUrl)
			...
			const data: DietCodeAuthApiTokenExchangeResponse = await response.json()
	...
	async signIn(
		...
		try {
			...
			const tokenUrl = new URL(DIETCODE_API_ENDPOINT.TOKEN_EXCHANGE, this.config.apiBaseUrl)
			...
			const responseType: DietCodeAuthApiTokenExchangeResponse = responseJSON
			...
			controller.stateManager.setSecret("dietcode:dietcodeAccountId", JSON.stringify(dietcodeAuthInfo))

			return dietcodeAuthInfo
		} catch (error) {
	...
	private async fetchRemoteUserInfo(tokenData: DietCodeAuthApiTokenExchangeResponse["data"]): Promise<DietCodeAccountUserInfo> {
		try {
			const userResponse = await axios.get(`${DietCodeEnv.config().apiBaseUrl}/api/v1/users/me`, {
				headers: {
			...
			return {
				id: tokenData.userInfo.dietcodeUserId || "",
